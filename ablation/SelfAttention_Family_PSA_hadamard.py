...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        B, L, H, E = queries.shape  # [B, C, H, D/H]
        _, S, _, D = values.shape   # [B, C, H, D/H]
                
        scale = self.scale or 1. / sqrt(E)        
        scores = torch.einsum("blhe,bshe->bhls", queries, keys)   # [B, H, C, C]
        # scores = queries + keys   # [B, C, H, D/H]
        ...
        V = torch.einsum("bhls,bshd->blhd", A, values)    # [B, C, H, D/H]
        # V = A + values   # [B, C, H, D/H]
...
class ProbAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):  # n_top: c*ln(L_q)
        ...
        ######################################
        Q_e = Q.unsqueeze(-2)                              # (B, H, L_Q, 1, D)
        K_e = K_sample                     # (B, H, 1, sample_k, D)
        # 哈达玛积后对 D 维求和
        Q_K_sample = torch.mul(Q_e, K_e).mean(dim=-1)               # (B, H, L_Q, sample_k)
        ######################################
        ...
        ######################################
        Qr_e = Q_reduce.unsqueeze(-2)                      # (B, H, n_top, 1, D)
        K_e  = K.unsqueeze(2)                             # (B, H, 1, L_K, D)
        Q_K  = torch.mul(Qr_e, K_e).mean(dim=-1)                    # (B, H, n_top, L_K)
        ######################################
        return Q_K, M_top
    ...
    def _update_context(self, context_in, V, scores, index, L_Q, attn_mask):
        ...
        ######################################
        # 先广播到 (B,H,n_top,L_K,D)，再 sum 掉 L_K 维
        weighted = attn.unsqueeze(-1).contiguous()
        V_exp = V.unsqueeze(2)              # -> (B, H, 1,     L_K, D)
        weighted = torch.mul(weighted, V_exp)            # -> (B, H, n_top, L_K, D)
        
        context_in[torch.arange(B)[:, None, None],
        torch.arange(H)[None, :, None],
        index, :] = weighted.mean(dim=-2)  # → (B,H,n_top,D)
        ######################################
