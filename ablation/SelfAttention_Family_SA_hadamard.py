...
class FullAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask, tau=None, delta=None):
        ...
        scores = torch.mul(queries, keys)   # [B, H, C, C]
        ...
        V = torch.mul(A, values)    # [B, C, H, D/H]
...
class ProbAttention(nn.Module):
    ...
    def _prob_QK(self, Q, K, sample_k, n_top):  # n_top: c*ln(L_q)
        ...
        Q_K_sample = (Q_e + K_e).mean(dim=-1)             # → (B, H, L_Q, sample_k)
        ######################################
        ...
        Q_K  = (Qr_e + K_e).mean(dim=-1)                # → (B, H, n_top, L_K)                           
        ######################################
        return Q_K, M_top
    ...
    def _update_context(self, context_in, V, scores, index, L_Q, attn_mask):
        ...
        ######################################
        A_e = attn.unsqueeze(-1)                     # (B, H, n_top, L_K, 1)
        V_e = V.unsqueeze(2)                         # (B, H, 1, L_K, D)
        A_add = A_e + V_e                            # (B, H, n_top, L_K, D)
        context_in[
            torch.arange(B)[:, None, None],
            torch.arange(H)[None, :, None],
            index, :
        ] = A_add.mean(dim=-2)                       # → (B, H, n_top, D)
        ######################################
